 ...
 *   storageAdapter: new S3StorageAdapter({ region: 'ap-south-1' }),
 *   cdnAdapter: new ImageKitCDNAdapter({ baseUrl: 'https://ik.imagekit.io/xxx' }),
 *   bucket: 'testimonials-dev-uploads',
 * });
 *
 * // Initialize upload
 * const result = await mediaService.initUpload({
 ...
 * });
 *
 * // Frontend uploads to result.uploadUrl
 * // Lambda validates and calls webhook
 * // API updates media record status
 *
 * // Get CDN URL for display
 * const url = mediaService.getMediaUrl(storagePath, { width: 200 });
 * ```
 */
